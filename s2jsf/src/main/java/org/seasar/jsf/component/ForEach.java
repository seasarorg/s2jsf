...
public class ForEach extends UIComponentBase implements NamingContainer {
    ...
    protected void restoreDescendantComponentStates(FacesContext context,
        ...
        for (int i = 0; i < children.size(); ++i) {
            ...
            if (child instanceof EditableValueHolder) {
                EditableValueHolder evh = (EditableValueHolder) child;
                EditableValueHolderState state = (EditableValueHolderState) descendantComponentStates
                        .get(child.getClientId(context));
                if (state != null) {
                    state.restore(evh);
                }
                /*
    ...
    public void initializeChildren(FacesContext context) {
        setupRows();
        for (int i = 0; i < rows.length; ++i) {
            setRowIndex(i);
            restoreDescendantComponentStates(context, this);
            initializeChildren0(context, this);
            saveDescendantComponentStates(context, this);
            removeVarAndVarIndex();
        }
    }

    private void initializeChildren0(FacesContext context, UIComponent component) {
        for (Iterator it = component.getChildren().iterator(); it.hasNext();) {
            ...
            initializeChildren0(context, child);
        }
    }
